...
def print_banner():
    print("\n" + "="*60)
    print("    OneShot WPS PIN Attack Tool - Versión Simplificada")
    print("    Herramienta para auditoría de seguridad WiFi")
    print("="*60)
    print("\n🔧 IMPORTANTE: Ejecutar como administrador/root")
...
def interactive_mode():
    ...
    while True:
        try:
            choice = input(f"\n🔧 Selecciona la interfaz a usar (1-{len(interfaces)}): ")
    ...
    print(f"\n✅ Interfaz seleccionada: {selected_interface}")
    
    # Seleccionar modo de ataque
    print("\n🎯 Modos de ataque disponibles:")
    print("   1. Pixie Dust (Recomendado - Rápido y efectivo)")
    print("   2. Fuerza Bruta (Lento pero completo)")
    print("   3. PIN Específico (Si conoces el PIN)")
    print("   4. Escaneo Automático (Busca redes vulnerables)")
    
    while True:
        try:
            mode = input("\n🔧 Selecciona el modo de ataque (1-4): ")
            if mode in ['1', '2', '3', '4']:
                break
            else:
                print("❌ Opción inválida. Intenta de nuevo.")
    ...
    args_dict = {'interface': selected_interface}
    
    if mode == '1':  # Pixie Dust
        print("\n🔥 Modo Pixie Dust seleccionado")
        print("   Este modo es rápido y efectivo contra routers vulnerables")
        args_dict['pixie_dust'] = True
        args_dict['write'] = True
        
    elif mode == '2':  # Fuerza Bruta
        print("\n💪 Modo Fuerza Bruta seleccionado")
        print("   Este modo puede tardar mucho tiempo")
        delay = input("   ⏱️  Delay entre intentos en segundos (recomendado: 1.5): ") or "1.5"
        try:
            args_dict['delay'] = float(delay)
        except:
            args_dict['delay'] = 1.5
        args_dict['bruteforce'] = True
        args_dict['write'] = True
        
    elif mode == '3':  # PIN específico
        pin = input("\n🔑 Ingresa el PIN WPS (8 dígitos): ")
        if len(pin) == 8 and pin.isdigit():
            args_dict['pin'] = pin
            args_dict['write'] = True
        else:
            print("❌ PIN inválido. Debe tener 8 dígitos.")
            return None
            
    elif mode == '4':  # Escaneo automático
        print("\n🔍 Modo Escaneo Automático seleccionado")
        print("   Se buscarán redes vulnerables automáticamente")
        args_dict['loop'] = True
        args_dict['pixie_dust'] = True
        args_dict['write'] = True
    
    # BSSID específico (opcional)
    bssid = input("\n📡 BSSID específico (opcional, formato: AA:BB:CC:DD:EE:FF): ").strip()
    if bssid and len(bssid) == 17:
        args_dict['bssid'] = bssid
    
    # Confirmación
    print("\n" + "="*50)
    print("📋 CONFIGURACIÓN FINAL:")
    print(f"   Interfaz: {args_dict['interface']}")
    if 'pixie_dust' in args_dict:
        print("   Modo: Pixie Dust")
    if 'bruteforce' in args_dict:
        print(f"   Modo: Fuerza Bruta (delay: {args_dict.get('delay', 1.5)}s)")
    if 'pin' in args_dict:
        print(f"   Modo: PIN específico ({args_dict['pin']})")
    if 'bssid' in args_dict:
        print(f"   BSSID: {args_dict['bssid']}")
    print("="*50)
    
    confirm = input("\n🚀 ¿Iniciar el ataque? (s/N): ").lower()
    if confirm != 's':
        print("\n❌ Operación cancelada")
        return None
...
def usage():
    return """
🔧 OneShot WPS PIN Attack Tool - Versión Mejorada

USO BÁSICO:
    python3 oneshot.py                    # Modo interactivo (recomendado)
    python3 oneshot.py -i wlan0 -K        # Pixie Dust rápido
    python3 oneshot.py -i wlan0 -B        # Fuerza bruta

ARGUMENTOS PRINCIPALES:
    -i, --interface=<wlan0>     : Interfaz WiFi a usar
    -b, --bssid=<mac>          : BSSID del AP objetivo
    -K, --pixie-dust           : Ataque Pixie Dust (recomendado)
    -B, --bruteforce           : Ataque de fuerza bruta
    -p, --pin=<pin>            : PIN específico a probar
    --interactive              : Modo interactivo (por defecto)

ARGUMENTOS AVANZADOS:
    -d, --delay=<n>            : Delay entre intentos [1.5]
    -w, --write                : Guardar credenciales en archivo
    -v, --verbose              : Salida detallada
    -l, --loop                 : Ejecutar en bucle
    --help                     : Mostrar esta ayuda

EJEMPLOS:
    python3 oneshot.py                           # Modo interactivo
    python3 oneshot.py -i wlan0 -K               # Pixie Dust automático
    python3 oneshot.py -i wlan0 -b AA:BB:CC:DD:EE:FF -K  # Pixie Dust específico
    python3 oneshot.py -i wlan0 -B -d 2          # Fuerza bruta con delay

⚠️  ADVERTENCIA: Solo usar en redes propias o con autorización explícita
"""
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--interactive',
        action='store_true',
        help='Run in interactive mode (default when no interface specified)'
        )
    parser.add_argument(
        '-b', '--bssid',
        type=str,
